...
public class ViewHelper<V extends IView, VM extends IViewModel<V>, VDB extends ViewDataBinding>
    ...
    public View inflate(@NonNull IExtendView<V, VM, VDB> extendView, @NonNull ViewStubProxy viewStubProxy) {
        perBind(extendView);
    ...
    public IViewModelStore getSelfViewModelStore() {
        IViewModelStore viewModelStore = viewBindingHelper.getSelfViewModelStore();
        if (ObjectUtils.nonNull(viewModelStore)) {
            return viewModelStore;
        }
        return extendView.getSelfViewModelStore();
    }
